...
abstract class BaseTableModel {
  ...
  * */
  Future<int> update({String where}) async {
    Database db = await DBManager.getDatabase();
    return await db?.update("$runtimeType", contentMap, where: where == null ? "id = ${id.content}" : where);
  }
  ...
  * */
  Future<int> delete({String where}) async {
    Database db = await DBManager.getDatabase();
    return await db?.delete("$runtimeType", where: where == null ? "id = ${id.content}" : where);
  }
  ...
  * */
  Future<BaseTableModel> one({String where = ""}) async {
    ...
    List<Map> data = await db.query(
      "$runtimeType",
      where: where.length > 0 ? where : "id=(select last_insert_id())",
      columns: columns,
    );
...
extension DataOption on BaseTableModel {
  ...
  * */
  String getSetContent(STSet obj){
    if(obj.setList.length <= 0 || obj.content == null || obj.content.length <= 0) return "";
    var value = "";
    obj.content.forEach((element) {
      if(element >= 0 && element < obj.setList.length){
        value += value.length > 0 ? ",${obj.setList[element]}" : "${obj.setList[element]}";
      }
    });
    return value;
  }
