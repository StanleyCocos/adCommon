...
class Auth {
  ...
  static Future<PermissionStatus> request({AuthType type, AuthStateCallback callback, AuthErrorCallback errorCallback}) async {
  ...
  static Future use({AuthType type, AuthStateCallback callback, AuthErrorCallback errorCallback}) async {
    PermissionStatus status = await request(type: type);
    if(status.isGranted){
      if(errorCallback != null) callback(status);
    } else {
      if(callback != null) errorCallback(status);
    }
  }
}
